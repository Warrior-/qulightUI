...
local function UpdateTooltip(element)
	...
	GameTooltip:SetText(isHonor and HONOR or COMBAT_XP_GAIN.." "..format(LEVEL_GAINED, UnitLevel("player")))
	GameTooltip:AddLine(" ")
	GameTooltip:AddLine(format(XP..": %s / %s (%d%%)", BreakUpLargeNumbers(cur), BreakUpLargeNumbers(max), perc), 0.75, 0.9, 1)
	GameTooltip:AddLine(string.format(LEVEL_ABBR..": %s (%d%%)", BreakUpLargeNumbers(max - cur), (max - cur) / max * 100 + 0.5), 0.75, 0.9, 1)

	if(rested > 0) then
		GameTooltip:AddLine(format('%s: %s (%d%%)', "|cff0090ff"..TUTORIAL_TITLE26, BreakUpLargeNumbers(rested), restedPerc), 0.75, 0.9, 1)
	end
