...
DebuffWhiteList = {
	...
	[SpellName(63106)] = true,	-- Сифффон
	[SpellName(27243)] = true,	-- Семка
	-- Warrior
...
local PlateBlacklist = {
	...
	["∆идкий обсидиан"] = true,
	["Lava Parasites"] = true,
	["Ћавовый паразит"] = true,
	-- Gundrak
	["Fanged Pit Viper"] = true,
	[" лыкаста€ глубинна€ гадюка"] = true,
}
...
local function CreateVirtualFrame(frame, point)
	...
	frame.bordertop:SetColorTexture(0.05, 0.05, 0.05, 1)
	...
	frame.borderbottom:SetColorTexture(0.05, 0.05, 0.05, 1)
	...
	frame.borderleft:SetColorTexture(0.05, 0.05, 0.05, 1)
	...
	frame.borderright:SetColorTexture(0.05, 0.05, 0.05, 1)
...
function Plates:UpdateHealthText()
	...
	if GetUnitName("target") and self.NewPlate:GetAlpha() == 1 then
		...
		self.NewPlate.CastBar.Icon:SetSize(((Qulight["nameplate"].height + Qulight["nameplate"].ad_height) * 2) + 8, ((Qulight["nameplate"].height + Qulight["nameplate"].ad_height) * 2) + 8)
		...
		self.NewPlate.CastBar.Icon:SetSize((Qulight["nameplate"].height * 2) + 8, (Qulight["nameplate"].height * 2) + 8)
...
function Plates:Skin(obj)
	...
	local NewPlate = self.Container[frame]
	NewPlate:SetSize(Qulight["nameplate"].width * noscalemult, (Qulight["nameplate"].height * noscalemult))
	...
	NewPlate.Health:SetStatusBarTexture(Qulight.media.texture)
	...
	NewPlate.Health.Background:SetTexture(Qulight.media.texture)
	...
	NewPlate.CastBar:SetStatusBarTexture(Qulight.media.texture)
	...
	NewPlate.CastBar.Icon:SetSize((Qulight["nameplate"].height * 2) + 8, (Qulight["nameplate"].height * 2) + 8)
	...
	if Qulight["nameplate"].show_castbar_name == true then
		...
		NewPlate.CastBar.Name:SetHeight(Qulight["media"].fontsize)
	...
	RaidTargetIcon:SetSize((Qulight["nameplate"].height * 2) + 8, (Qulight["nameplate"].height * 2) + 8)

	if Qulight["nameplate"].track_auras == true then
		if not NewPlate.icons then
			NewPlate.icons = CreateFrame("Frame", nil, NewPlate.Health)
			NewPlate.icons:SetPoint("BOTTOMRIGHT", NewPlate.Health, "TOPRIGHT", 0, 15)
	...
	if Qulight["nameplate"].class_icons == true then
		...
		NewPlate.class:SetPoint("TOPRIGHT", NewPlate.Health, "TOPLEFT", -5, noscalemult * 2)
		NewPlate.class:SetTexture("Interface\\WorldStateFrame\\Icons-Classes")
		NewPlate.class:SetSize((Qulight["nameplate"].height * 2) + 11, (Qulight["nameplate"].height * 2) + 11)
...
local function CreateAuraIcon(parent)
	...
	button.bord = button:CreateTexture(nil, "BORDER")
	button.bord:SetColorTexture(0.05, 0.05, 0.05, 1)
	button.bord:SetPoint("TOPLEFT", button, "TOPLEFT", 0, 0)
	button.bord:SetPoint("BOTTOMRIGHT", button, "BOTTOMRIGHT", 0, 0)
...
local function AuraFilter(caster, spellname)
	if caster == "player" then
		if Config["myfiltertype"] == "none" then
			return false
		elseif Config["myfiltertype"] == "whitelist" and DebuffWhiteList[spellname] then
			return true
		...
		if Config["otherfiltertype"] == "none" then
			return false
		elseif Config["otherfiltertype"] == "whitelist" and DebuffWhiteList[spellname] then
			return true
		end
	end
end
...
local function UpdateBuffs(unitFrame)
	...
	if not Qulight["nameplate"].track_auras and UnitIsUnit(unitFrame.displayedUnit, "player") then return end
...
-- if Config.playerplate then
	...
	PowerFrame.powerBar:SetStatusBarTexture(Qulight.media.texture)
...
-- if Config.classresource_show then
	...
	for i = 1, 6 do
		...
		if UnitClass("player") == "DEATHKNIGHT" then
			...
			Resourcebar[i].value:SetFont(Qulight["media"].font, 10, "THINOUTLINE")
			Resourcebar[i].value:SetShadowOffset(1, -1)
	...
	Resourcebar:SetScript("OnEvent", function(self, event, unit, powerType)
		...
		if event == "PLAYER_TALENT_UPDATE" then
			if multicheck(UnitClass("player"), "WARLOCK", "PALADIN", "MONK", "MAGE", "ROGUE", "DRUID") and not RequireSpec or RequireSpec == GetSpecialization() then -- 
		...
		elseif event == "PLAYER_ENTERING_WORLD" or (event == "UNIT_POWER_FREQUENT" and unit == "player" and powerType == ClassPowerType) then
			if multicheck(UnitClass("player"), "WARLOCK", "PALADIN", "MONK", "MAGE", "ROGUE", "DRUID") then
				local cur, max, oldMax

				cur = UnitPower('player', ClassPowerID)
				max = UnitPowerMax('player', ClassPowerID)

				if multicheck(UnitClass("player"), "WARLOCK", "PALADIN", "MONK", "MAGE") then
					for i = 1, max do
						if(i <= cur) then
							self[i]:Show()
						else
		...
		elseif UnitClass("player") == "DEATHKNIGHT" and event == "RUNE_POWER_UPDATE" then
			local rid = unit
...
local function UpdateName(unitFrame)
	...
	if name then
		...
		if Qulight["nameplate"].class_icons == true and UnitIsPlayer(unitFrame.displayedUnit) then
...
local function UpdateHealth(unitFrame)
	...
	elseif not UnitIsPlayer(unit) and Qulight["nameplate"].enhance_threat == true then
		SetVirtualBorder(unitFrame.healthBar, 0.05, 0.05, 0.05, 1)
	end
end
...
local function UpdateHealthColor(unitFrame)
	...
	if not UnitIsConnected(unit) then
		...
		local classColor = oUF_colors.class[class]

		if UnitIsUnit("player", unit) then
			r, g, b = unpack(oUF_colors.class[class])
...
function NamePlates_UpdateNamePlateOptions()
	...
	local baseNamePlateWidth = Qulight["nameplate"].width * noscalemult
...
local function OnNamePlateCreated(namePlate)
	...
	namePlate.UnitFrame.healthBar:SetHeight(Qulight["nameplate"].height * noscalemult)
	namePlate.UnitFrame.healthBar:SetPoint("LEFT", 0, 0)
	namePlate.UnitFrame.healthBar:SetPoint("RIGHT", 0, 0)
	namePlate.UnitFrame.healthBar:SetStatusBarTexture(Qulight.media.texture)
	...
	namePlate.UnitFrame.healthBar.Background:SetTexture(Qulight.media.texture)
	...
	if Qulight["nameplate"].health_value == true then
		...
		namePlate.UnitFrame.healthBar.value:SetFont(Qulight["media"].font, 8, "THINOUTLINE")
		namePlate.UnitFrame.healthBar.value:SetShadowOffset(1, -1)
	...
	namePlate.UnitFrame.name:SetFont(Qulight["media"].font, 8, "THINOUTLINE")
	namePlate.UnitFrame.name:SetShadowOffset(1, -1)
	...
	namePlate.UnitFrame.level:SetFont(Qulight["media"].font, 8, "THINOUTLINE")
	namePlate.UnitFrame.level:SetShadowOffset(1, -1)
	...
	namePlate.UnitFrame.castBar:SetHeight(Qulight["nameplate"].height * noscalemult)
	namePlate.UnitFrame.castBar:SetPoint("TOPRIGHT", namePlate.UnitFrame.healthBar, "BOTTOMRIGHT", 0, -8)
	namePlate.UnitFrame.castBar:SetPoint("BOTTOMLEFT", namePlate.UnitFrame.healthBar, "BOTTOMLEFT", 0, -8-(Qulight["nameplate"].height * noscalemult))
	namePlate.UnitFrame.castBar:SetStatusBarTexture(Qulight.media.texture)
	...
	namePlate.UnitFrame.castBar.Time:SetFont(Qulight["media"].font, 8, "THINOUTLINE")
	namePlate.UnitFrame.castBar.Time:SetShadowOffset(1, -1)
	...
	if Qulight["nameplate"].show_castbar_name == true then
		...
		namePlate.UnitFrame.castBar.Text:SetFont(Qulight["media"].font, 8, "THINOUTLINE")
		namePlate.UnitFrame.castBar.Text:SetShadowOffset(1, -1)
		namePlate.UnitFrame.castBar.Text:SetTextColor(1, 1, 1)
		namePlate.UnitFrame.castBar.Text:SetHeight(Qulight["media"].fontsize)
	...
	namePlate.UnitFrame.castBar.Icon:SetSize((Qulight["nameplate"].height * 2 * noscalemult) + 8, (Qulight["nameplate"].height * 2 * noscalemult) + 8)
	...
	namePlate.UnitFrame.RaidTargetFrame:SetSize((Qulight["nameplate"].height * 2) + 8, (Qulight["nameplate"].height * 2) + 8)
	namePlate.UnitFrame.RaidTargetFrame:SetPoint("BOTTOM", namePlate.UnitFrame.name, "TOP", 0, Qulight["nameplate"].track_auras == true and 38 or 16)
	...
	if Qulight["nameplate"].track_auras == true then
		...
		namePlate.UnitFrame.icons:SetPoint("BOTTOMRIGHT", namePlate.UnitFrame.healthBar, "TOPRIGHT",  0, 15)
		namePlate.UnitFrame.icons:SetWidth(20 + Qulight["nameplate"].width)
		namePlate.UnitFrame.icons:SetHeight(Qulight["nameplate"].auras_size)
