...
SlashCmdList.MOUSEOVERBIND = function()
	...
	if not bind.loaded then
		...
		GameTooltip:SetBackdropBorderColor(0.37, 0.3, 0.3, 1)
		...
		function bind:Update(b, spellmacro)
			...
			if spellmacro == "SPELL" then
				...
				GameTooltip:SetScript("OnHide", function(self)
					...
					if #bind.button.bindings == 0 then
						self:AddLine("No bindings set", 0.6, 0.6, 0.6)
					else
						self:AddDoubleLine("Binding", "Key", 0.6, 0.6, 0.6, 0.6, 0.6, 0.6)
			...
			elseif spellmacro == "MACRO" then
				...
				bind.button.bindings = {GetBindingKey(spellmacro.." "..bind.button.name)}
					if #bind.button.bindings == 0 then
						GameTooltip:AddLine("No bindings set", 0.6, 0.6, 0.6)
					else
						GameTooltip:AddDoubleLine("Binding", "Key", 0.6, 0.6, 0.6, 0.6, 0.6, 0.6)
						for i = 1, #bind.button.bindings do
		...
		function bind:Listener(key)
			...
			if key == "ESCAPE" or key == "RightButton" then
				...
				print("|cffffff00".."All keybindings cleared for".."|r".." |cff00ff00"..self.button.name.."|r|cffffff00.|r")
				self:Update(self.button, self.spellmacro)
				if self.spellmacro ~= "MACRO" then GameTooltip:Hide() end
				return
			end
		...
		function bind:Deactivate(save)
			...
			if save then
				SaveBindings(which)
				print("|cffffff00".."All keybindings have been saved.".."|r")
			else
				LoadBindings(which)
				print("|cffffff00".."All newly set keybindings have been discarded.".."|r")
			end
			self.enabled = false
			self:HideFrame()
